...
COPY . .
RUN dotnet build "ConfigService.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ConfigService.csproj" \
...
Execute unit tests
```
    cd "ConfigService/test/ConfigService.Tests"
    dotnet test
```

Deploy application
```
    cd "ConfigService/src/ConfigService"
